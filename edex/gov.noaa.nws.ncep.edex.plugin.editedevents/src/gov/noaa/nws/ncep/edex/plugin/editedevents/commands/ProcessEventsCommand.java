...
import gov.noaa.nws.ncep.edex.plugin.editedevents.dao.GoesXrayEventDao;
 ...
 * @version 1.0	
 */
public class ProcessEventsCommand extends BaseCommand {
	 ...
	 * Logger
	 */
	 private static final IUFStatusHandler statusHandler = UFStatus.getHandler(ProcessEventsCommand.class);

	public ProcessEventsCommand() {
	...
	public IResponse execute() {
		...
		try {
			...
			if (this.request.isProcessOtherEvents()) {
				...
				statusHandler.handle(Priority.INFO, " EDITEDEVENTS.ProcessEventsCommand results = " + resultsSize);
				...
				for (int i = 0; i < resultsSize; i++) {
					...
					if (map != null) {
						...
						try {
							...
							statusHandler.handle(Priority.INFO, " EDITEDEVENTS.ProcessEventsCommand: DataIntegrityViolationException occured");
						}
					}	
				}	
			}
			...
			statusHandler.handle(Priority.INFO, " EDITEDEVENTS.ProcessEventsCommand.execute() complete.");


		} catch (Exception e) {
			error = new EditedEventsException(
					"ERROR - Exception occured while executing the ProcessEventsCommand.",
					e);
			e.printStackTrace();
		}
	...
	private Event createEventRecord(Map<String, Object> map) {
		...
		statusHandler.handle(Priority.INFO, " EDITEDEVENTS.ProcessEventsCommand.createEventRecord()");
		...
		try {		
			...
			if (tmp != null) {
				...
				event.setBeginTime(Integer.valueOf((beginDTTM
						.get(Calendar.HOUR_OF_DAY) + "" + EECommandUtil.getTwoDigitValueAsString(beginDTTM
						.get(Calendar.MINUTE)))));
			} else {
			...
			if (tmp != null) {
				...
				event.setEndTime(Integer.valueOf((endDTTM.get(Calendar.HOUR_OF_DAY)
						+ "" + EECommandUtil.getTwoDigitValueAsString(endDTTM.get(Calendar.MINUTE)))));
			} else {
			...
			if (tmp != null) {
				...
				event.setMaxTime(Integer.valueOf((maxDTTM.get(Calendar.HOUR_OF_DAY)
						+ "" + EECommandUtil.getTwoDigitValueAsString(maxDTTM.get(Calendar.MINUTE)))));
			} else {
			...
			String maxClass = ((String) map.get(GOESXrayEvent.MAX_CLASS) != null) ? (String) map
			...
			double maxTemp =  (double) ((map.get(GOESXrayEvent.MAX_TEMP) != null) ?  map
					.get(GOESXrayEvent.MAX_TEMP) : 0.0);
			double maxEmissionMeas = (double) ((map.get(GOESXrayEvent.MAX_EMISSION_MEAS) != null) ?  map
					.get(GOESXrayEvent.MAX_EMISSION_MEAS) : 0.0);
			double maxXrLong = (double) (( map.get(GOESXrayEvent.MAX_XR_LONG) != null) ?  map
			...
			event.setParticulars2(this.format("0.00E00", String.valueOf(curIntXrLong))); 
	
			// Round to Integer
			event.setParticulars3(this.format("######0", String.valueOf(maxTemp)));
	
			// 4 Sigfigs Two To Right of Decimal
			event.setParticulars4(this.format("00.00", String.valueOf(maxEmissionMeas)));
			...
			statusHandler.handle(Priority.INFO, " EDITEDEVENTS: Error occured at ProcessEventsCommand.createEventRecord.", e);
		}
	...
	private void applyCORandDELRules(Event event) throws PluginException,
		...
		statusHandler.handle(Priority.INFO, " EDITEDEVENTS.ProcessEventsCommand.applyCORandDELRules()");
		...
		statusHandler.handle(Priority.INFO, " EDITEDEVENTS.ProcessEventsCommand.applyCORandDELRules(), dupEvents.size = " + ((dupEvents != null)?dupEvents.size():0));

		statusHandler.handle(Priority.INFO, " EDITEDEVENTS.ProcessEventsCommand.applyCORandDELRules(), event status code = " +event.getStatusCd());
	...
	private void applyDELRule(List<Event> dupEvents) {
		...
		for (int i = 0; i < dupEventsSize; i++) {
			...
			dupEvent.setStatusText(EditedEventsConstants.EVENT_AGE.DEL.toString());
	...
	public String getParticulars10(String value) {
		...
		result = String.format("%e",
				...
				+ String.format("%e", Double.parseDouble(value.substring(
	 ...
	 * 
	 * @see
	 * gov.noaa.nws.ncep.edex.plugin.editedevents.commands.intf.ICommand#setRequest
