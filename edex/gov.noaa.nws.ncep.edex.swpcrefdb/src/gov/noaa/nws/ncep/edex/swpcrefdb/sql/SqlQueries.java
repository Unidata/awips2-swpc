...
public abstract class SqlQueries {
    ...
    public static final String GET_COMPLETE_GOES_TRACKING_STATUS = "SELECT s.designation satellite "
            + ", s.name satellite_name " + ", i.description instrument "
            + ", sts.description tracking_status " + ", ts.begin_dttm "
            + ", ts.end_dttm " + ", to_timestamp(to_char(ts.begin_dttm, " + "'"
            + "YYYY-MM-DD" + "'" + ")," + "'" + "YYYY-MM-DD HH24:MI:SS.ss" + "'"
            + ") begin_day " + ", to_timestamp(to_char(ts.end_dttm, " + "'"
            + "YYYY-MM-DD" + "'" + "), " + "'" + "YYYY-MM-DD HH24:MI:SS.ss"
            + "'" + ") end_day " + "FROM swpc_station s "
    ...
    /**
     * first parameter is X-rays second parameter is Primary 3-8 parameters are
     * some date values 9 parameter is X-rays 10 parameter is Secondary 11-16
     * parameters are some date values 17 parameter is X-rays 18 parameter is
     * Tertiary 19-24 parameters are some date values
     */
    public static final String GET_GOES_PRIMARY_SECONDARY_TERTIARY_SATELLITES = "SELECT satellite, "
            + "tracking_status," + "begin_dttm," + "end_dttm from ("
            + SqlQueries.GET_COMPLETE_GOES_TRACKING_STATUS
            + ") completeGoesTrackingStatus "
            ...
            + "AND ((completeGoesTrackingStatus.end_day - cast(" + "'" + "1 day"
            + "'" + " as interval)) BETWEEN ? AND ? "
            + "OR (completeGoesTrackingStatus.end_day - cast(" + "'" + "1 day"
            + "'" + " as interval)) >= ? "
            + "OR completeGoesTrackingStatus.end_day IS NULL) " + "OR "
    ...
    /**
     * first parameter is instrument e.g. "X-rays" second parameter is tracking
     * status e.g. "Primary", "Secondary", or "Tertiary" 3-8 parameters are some
     * date values
     */
    public static final String GET_GOES_SATELLITE = "SELECT satellite from ("
            + SqlQueries.GET_COMPLETE_GOES_TRACKING_STATUS
            + ") completeGoesTrackingStatus "
            + "WHERE (completeGoesTrackingStatus.instrument = ? "
            + "AND completeGoesTrackingStatus.tracking_status = ? "
     ...
     * date values
     */
    public static final String GET_GOES_SATELLITE_FOR_INTEGRATED_FLUX = "SELECT satellite from ("
            + SqlQueries.GET_COMPLETE_GOES_TRACKING_STATUS
            + ") completeGoesTrackingStatus "
            + "WHERE completeGoesTrackingStatus.instrument = ? "
            + "AND completeGoesTrackingStatus.tracking_status = ? "
     ...
     * processed=true first parameter is begin date time second parameter is
