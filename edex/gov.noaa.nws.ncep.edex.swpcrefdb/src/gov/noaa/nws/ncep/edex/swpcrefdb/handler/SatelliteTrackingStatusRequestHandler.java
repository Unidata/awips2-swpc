 ...
 * @version 1.0	
 */
public class SatelliteTrackingStatusRequestHandler implements IRequestHandler<SatelliteTrackingStatusRequest>  {
	...
	public Object handleRequest(SatelliteTrackingStatusRequest request)
		...
		SatelliteTrackingStatusRequestType requestType = request.getRequestType();
		
		switch (requestType) {
			case GET_GOES_SATELLITE:
				return trackingStatusDao.getGOESSatellite(request.getInstrument(), request.getTrackingStatus(), request.getBeginDTTM(), request.getEndDTTM());	
			case GET_GOES_SATELLITE_FOR_INTEGRATED_FLUX:
				return trackingStatusDao.getGOESSatelliteForIntFlux(request.getInstrument(), request.getTrackingStatus(), request.getBeginDTTM(), request.getEndDTTM());			
			case GET_GOES_SATELLITES:				
				return trackingStatusDao.getGOESSatellite(request.getInstrument(), request.getBeginDTTM(), request.getEndDTTM());
			default:
				break;
		}
