...
/**
 * Dialog class to allow the user to choose date and time
 * 
 * <pre>
 * SOFTWARE HISTORY
 * Date         Ticket#    Engineer    Description
 * ------------ ---------- ----------- --------------------------
 * Jan 2015     R9583      sgurung   Initial creation.
 * 
 * </pre>
 * 
 * @author sgurung
...
public class CalendarDialog extends Dialog {
	...
	public Calendar open() {
		...
		return open(current, TimeZone.getTimeZone("GMT"));
	}

	// return the selected calendar or null on Cancel.
	public Calendar open( Calendar initDateTime, TimeZone tz) {

		if( initDateTime == null ) {
			initCal = Calendar.getInstance();
		} else {
			initCal = initDateTime;
		}
		
		initCal.setTimeZone(tz);
    	...
    	fd.left = new FormAttachment( 50, -90 ); // center
	...
	public void updateSelectedDate( ) {
		...
		seldDateTxt.setText(selectedCal.getTime().toString());
	}
}